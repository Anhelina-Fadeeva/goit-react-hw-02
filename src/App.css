...
function App() {
  ...
  const positiveFeedback=Math.round(((comment.good + comment.neutral) / totalFeedback) * 100);
  ...
  const updateFeedback=feedbackType=>setComment(prev=> ({
      ...prev, [feedbackType]: prev[feedbackType] + 1
    }));

const ressetFeedback=()=>setComment({
  good: 0,
  neutral: 0,
  bad: 0,
});

return (<> <Description /> <Options updateFeedback= {
    updateFeedback
  }
  ...
  /> {
    totalFeedback > 0 && (<Feedback comment= {
        comment
      }
